...
def _generate_lua_file(participants: List[Participant]) -> None:
    from util import get_lua_path
    ...
    for participant in participants:
        ...
        for idx, waypoint in enumerate(participant.movement[0:-1]):
            ...
            def generate_lua_function() -> str:
                ...
                lua_lines.extend(_generate_lua_av_command(participant, idx, waypoint.mode, current_movement_mode))
                lua_lines.extend([
                    "  end",
                    "end",
                    "",
...
def _enable_participant_movements(participants: List[Participant]) -> None:
    """
    ...
    """
    _generate_lua_file(participants)
    _add_lua_triggers(participants)


def _make_lanes_visible() -> None:
    """
...
def _add_lap_config(waypoint_ids: Set[str]) -> None:
    """
